...
class JavaGuard extends EventEmitter {
  ...
  static loadMojangLauncherData() {
    return new Promise((resolve, reject) => {
      request.get(PATHS.LAUNCHERDATA, (err, resp, body) => {
        if (err) {
          resolve(null);
        } else {
...
class AssetGuard extends EventEmitter {
  ...
  loadVersionData(version, force = false) {
    ...
    return new Promise(async (resolve, reject) => {
      ...
      if (!fs.existsSync(versionFile) || force) {
        ...
        stream.on("finish", () => {
          console.log("1188", versionFile);
          resolve(JSON.parse(fs.readFileSync(versionFile)));
        });
      } else {
        console.log("1192", ">", versionFile, "<");
        resolve(JSON.parse(fs.readFileSync(versionFile)));
      }
    });
  }
  ...
  _getVersionDataUrl(version) {
    return new Promise((resolve, reject) => {
      request(PATHS.VERSION_MANIFEST, (error, resp, body) => {
        if (error) {
          reject(error);
        } else {
  ...
  _assetChainValidateAssets(versionData, indexData) {
    ...
    return new Promise((resolve, reject) => {
      //Asset constants
      const resourceURL = PATHS.RESOURCE_URL;
